...
readonly RMAN_FILE=${SCRIPT_DIR}/weekly/level0.weekly.rman
readonly LOG_FILE=${BASE_DIR}/logs/level0.weekly.${DATE_NOW}.log
...
if [ $? -eq 0 ]; then
  echo "RMAN weekly level 0 backup script  finished successfully.";
  exit 0
else
  echo "RMAN weekly level 0 backup script failed.";
  exit 1
fi